...
```bash
mise run install_hooks   # once per clone
```

Writes `.git/hooks/pre-commit`, which runs `mise run pre-commit` — the same lint
and test tasks CI runs, so a commit that passes locally is not one CI rejects on
formatting or types. It takes about 1.2s on this repo, the four tasks running in
parallel. Git does not version hooks, hence the one-time install per clone.

Generated by `mise generate git-pre-commit`: no extra tool and no second config
format. lefthook, prek and hk are all good at filtering large file sets and
orchestrating many linters — neither of which this repo needs, and each would
mean redefining the commands alongside `mise.toml`.
